        ...
        <div className="flex items-center justify-between flex-wrap">
          ...
          <div className="flex md:flex-row flex-col items-center gap-3">
            ...
            <label htmlFor="thumbnailImage" className="flex items-center gap-3">
              ...
              {image && (
                <img className="max-h-10" src={URL.createObjectURL(image)} alt="image" />
              )}
            </label>
          </div>
        </div>
                ...
                <div className="p-4">
                  {chapter.chapterContent.map((lecture, lectureIndex) => (
                    <div key={lectureIndex} className="flex justify-between items-center mb-2">
                      <span>
                        {lectureIndex + 1} {lecture.lectureTitle} - {lecture.lectureDuration} mins - <a href={lecture.lectureUrl} target="_blank" className="text-blue-500">Link</a> -
                        {lecture.isPreviewFree ? 'Free Preview' : 'Paid'}
                      </span>
                      <img src={assets.cross_icon} alt="cross" className="cursor-pointer" onClick={() => handleLecture('remove', chapter.chapterId, lectureIndex)} />
                    </div>
                  ))}
          ...
          {showPopup && (
            <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
              <div className="bg-white text-gray-700 p-4 rounded relative w-full max-w-80">
                ...
                <img onClick={() => setShowPopup(false)} src={assets.cross_icon} alt="cross" className="top-4 absolute right-4 w-4 cursor-pointer" />
